...
$("#search-table").submit(function (e) {
  ...
  Request(Endpoints.get("searchTemplate"), "POST", data, { showMainLoader: true }).done(resData => {
...
//     loggedInUserName: User.getName()
//   };
//   var senderIDSelect = document.getElementById("userName");
//   Request(Endpoints.get("getAllChildsForUser"), "POST", data, {
//     showMainLoader: true
//   }).done(data => {
//     const gpList = data.data.clientList;
//     // listSenderID = gpList;
//     $.each(gpList, function (key, value) {
//       senderIDSelect.options[senderIDSelect.options.length] = new Option(value, value);
//     });
//   });
    ...
    $('#' + id + ' tbody').on('click', 'button', function () {
      ...
      Request(Endpoints.get("searchTemplate"), "POST", formData).done(resData => {
        if (Endpoints.validateResponse(resData)) {
          ...
          $("#editModal #templateTitle").val(data[5]);
          $("#editModal #templateDescription").val(data[6]);
          $("#editModal #templateText").val(data[7]);
          $("#editModal #entityIdModal").val(data[10]);
        }
      });
    });
  });

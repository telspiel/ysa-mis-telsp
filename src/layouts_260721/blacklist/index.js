...
$("#submitSearch").click(() => {
  ...
  Request(Endpoints.get("searchUserBlacklistNumber"), "POST", data).done(data1 => {
    if (data1.data) {
...
$("#remove-gblacklist").submit(function (e) {
  ...
  Request(
    Endpoints.get("removeNumberFromUserBlackList"),
    "POST",
    $.extend({}, formData, additionalData)
  ).done(data => {
...
function loadAddBlacklistNumbersTable() {
  ...
  Request(Endpoints.get("getAllBlacklistNumbersForUser"), "POST", data).done(data1 => {
    $("#dwnloadBtn").html(data1.data.downloadReportLink);
    getDownloadableFile();
    $("#dwnloadBtn #report_donwload").addClass("btn btn-primary");
    if (!Endpoints.validateResponse(data1)) {
